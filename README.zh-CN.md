...
安装脚本会自动判断服务器环境：如果 80/443 已经由 Nginx 占用，会把 Manager 启动在 `127.0.0.1:8080` 并写入 Nginx 反代配置；如果没有现成 Nginx，会启用内置 Caddy 自动申请 HTTPS。

## 使用本地电脑的 SOCKS5

服务器里的浏览器不能直接访问你本机的 `127.0.0.1:10808`。如果要让服务器浏览器使用本机 Xray/Clash 的 SOCKS5，需要保持本机代理和 SSH 反向隧道同时运行。

隧道接通后，在配置的代理协议中填写：

```text
socks5://host.docker.internal:19081
```

本机电脑关机、休眠、Xray 停止或隧道断开时，服务器上的这个代理就会失效。若希望完全不依赖本机，应改用服务器可直接访问的 SOCKS5 或 VPN。
